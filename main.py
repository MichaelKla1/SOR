matrixA = [[3,-1,1],[-1,3,-1],[1,-1,3]]
vectorB = [-1,7,-7]

epsilon = 0.00001
numOfIterations = 100 #max number of iterations
w = 1.15 #omega for SOR method
    ...
    while i < numOfIterations:
        ...
        while j < len(variables):
            ...
            while k < len(variables):
                ...
                k += 1
            result *= w
            variables[j] = result + ((1 - w) * variables[j])
            j += 1
        # check if X(i)r+1 - X(i)r <=epsilon
        k = 0
        flag = True
        while k < len(variables):
            if abs(variables[k] - tempvariables[k]) > epsilon:
                flag = False
                break
            k += 1
        if(flag):
